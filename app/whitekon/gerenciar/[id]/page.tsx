...
import { Switch } from "@/components/ui/switch"
import { WhiteKonStorage } from "@/lib/whitekon-storage"
import { type WhiteKon, type WhiteKonData } from "@/lib/types"
import { useToast } from "@/hooks/use-toast"
...
export default function GerenciarWhiteKonPage() {
  ...
  const loadParametersFromDevice = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "Erro ao Sincronizar com Dispositivo", description: error.message, variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleManualControlToggle = async (checked: boolean) => {
    setIsLoading(true);
    const newModeValue = checked ? 1 : 0;
    
    toast({ title: `Enviando comando: ${checked ? 'HABILITAR' : 'DESABILITAR'} Controle Manual...` });
    
    const success = await writeRegister(29, newModeValue);

    if (success) {
      toast({ title: "Comando recebido. Verificando estado final do dispositivo..." });
      await new Promise(resolve => setTimeout(resolve, 500)); // Pausa para o dispositivo processar
      await loadParametersFromDevice(); // **PASSO CRÍTICO: Relê os dados para confirmar a mudança**
    } else {
      toast({ title: "Erro", description: "O dispositivo não aceitou o comando.", variant: "destructive" });
      await loadParametersFromDevice(); // Sincroniza a UI com o estado real, que não mudou
    }
  ...
  const handleDebugCommand = async (command: "led" | "bobina", value: boolean) => {
    ...
    try {
      ...
      const success = await writeRegister(28, newValue);
      if (success) {
        if (command === "led") setLedStatus(value);
        if (command === "bobina") setBobinStatus(value);
        toast({ title: "Comando executado" });
      } else {
